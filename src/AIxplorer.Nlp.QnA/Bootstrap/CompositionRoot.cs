...
using AIxplorer.Nlp.QnA.Services;

namespace AIxplorer.Nlp.QnA.Bootstrap;
...
public class CompositionRoot
{
    ...
    private void RegisterServices(ILogger<CompositionRoot> logger, IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton<QuestionAnsweringAssistant>(sp =>
        {
            var loggerForAssistant = sp.GetRequiredService<ILogger<QuestionAnsweringAssistant>>();
            return new QuestionAnsweringAssistant(loggerForAssistant, modelPath);
        });

        services.AddScoped<IQuestionAnsweringService, QuestionAnsweringServiceImpl>();
    }
}
